...
describe('ShopeeApi Test', function () {
  ...
  it('should able to validate webhook signature', function () {
    const mockSignature = '43d0866e15622bda257475a6ad55348d1cb2c9653c2feb54cac58a19cfbb3db3'
    const mockParams = {
      msg: 'some msg'
    }
    const shopeeApi = new ShopeeApi(options)
  ...
  it('should callback on make request', function () {
    ...
    nock('https://partner.shopeemobile.com').post('/api/v1/shop/get_partner_shop').reply(200, expectedBody)
  ...
  it('should return promise on make request', function () {
    ...
    const shopeeApi = new ShopeeApi(options)
    return shopeeApi.post('/shop/get_partner_shop', {}).then(function ({ body, res }) {
      expect(res.statusCode).to.equal(200)
      expect(JSON.stringify(body)).to.equal(JSON.stringify(expectedBody))
    })
  })
  ...
  it('should return error callback when error', function () {
    ...
    nock('https://partner.shopeemobile.com').post('/api/v1/shop/get_partner_shop').replyWithError(expectedBody)
